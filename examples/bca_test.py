...
def _main():
    ...
    if not (args.model == 'md_dnn' or args.model == 'kde') and \
            (args.model == 'at_amd_pad' and hp_params['detector'] is None):
    ...
    if 'indicator' in type(model).__dict__.keys():
        indicator_flag = model.indicator(np.mean(np.concatenate(x_density_list)), y_pred)
