...
def _main():
    ...
    attack = BCA(device=model.device,
                 kappa=args.kappa
                 )
    if args.ascending:
        interval = 10
    ...
    for m in range(interval, args.n_pertb + 1, interval):
        ...
        if 'indicator' in type(model).__dict__.keys():
            ...
            logger.info(f'The mean accuracy on adversarial malware (W/ indicator) is {sum((y_pred == 1.) & indicator_flag) / np.sum(indicator_flag) * 100:.3f}%.')
