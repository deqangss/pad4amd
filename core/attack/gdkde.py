...
class GDKDE(BaseAttack):
    """
    a variant of gradient descent with kernel density estimation: we calculate the density estimation upon the hidden space
    ...
    def __init__(self, ben_hidden=None, bandwidth=20., kappa=10., manipulation_z=None, omega=None, device=None):
        super(GDKDE, self).__init__(manipulation_z, omega, device)
    ...
    def get_losses(self, model, logit, label, hidden):
        ...
        if 'forward_g' in type(model).__dict__.keys():
            ...
            # loss_no_reduction += 1e10 * (torch.clamp(
