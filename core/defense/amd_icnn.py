    ...
    def fit(self, train_data_producer, validation_data_producer, epochs=100, lr=0.005, weight_decay=0., verbose=True):
        ...
        for i in range(epochs):
            ...
            for idx_batch, (x_train, y_train) in enumerate(train_data_producer):
                ...
                x_train_noises = torch.clamp(x_train + utils.psn(x_train, np.minimum(np.random.uniform(0, 1.), 0.5)),
                                             min=0., max=1.)
            ...
            with torch.no_grad():
                for x_val, y_val in validation_data_producer:
                    ...
                    x_val_noises = torch.clamp(x_val + utils.psn(x_val, np.minimum(np.random.uniform(0, 1.), 0.5)),
                                               min=0., max=1.)
