...
class AMalwareDetectionPAD(object):
    ...
    def fit2(self, train_data_producer, validation_data_producer=None, mimicry_attack_example=None ,
             adv_epochs=50,
             beta_1=0.1,
             beta_2=1,
             lmda_lower_bound=1e-3,
        ...
        ckpt = torch.load(self.model_save_path + '.tmp')
        epoch = ckpt['epoch']
        ...
        acc_val_adv_be = 0.
        best_epoch = 0
        for i in range(epoch, epoch + adv_epochs):
            random.seed(0)
            losses, accuracies = [], []
            mal_count = 0
            for idx_batch, (x_batch, y_batch) in enumerate(train_data_producer):
                ...
                disc_pertb_mal_x_ = utils.round_x(pertb_mal_x, 0.5)
                total_time += time.time() - start_time
                mal_num = len(mal_x_batch)
                print(mal_count,mal_num,  mimicry_attack_example.shape)
                x_batch = torch.cat([x_batch, mimicry_attack_example[mal_count: mal_count+mal_num],
                                     disc_pertb_mal_x_], dim=0)
                y_batch = torch.cat([y_batch, mal_y_batch, mal_y_batch])
                mal_count += mal_num
                if use_continuous_pert:
                    filter_flag = torch.amax(torch.abs(pertb_mal_x - mal_x_batch), dim=-1) <= 1e-6
                    pertb_mal_x = pertb_mal_x[~filter_flag]
                ...
                if verbose:
                    print(
                        f'Mini batch: {i * nbatches + idx_batch + 1}/{(epoch + adv_epochs) * nbatches} | training time in {mins:.0f} minutes, {secs} seconds.')
                if hasattr(self.model, 'forward_g'):
                    acc_g_train = ((torch.sigmoid(logits_g) >= 0.5) == y_batch_).sum().item()
                    acc_g_train /= x_batch_.size()[0]
                    accuracies.append(acc_g_train)
