...
class MalwareDetectorIndicator(MalwareDetector):
    ...
    def energy(self, representation, logits):
        ...
        # E_z = torch.sum(torch.log(prob_n * self.phi + exp_over_flow) * weights, dim=1)
        E_z = torch.sum(gamma_z * torch.log(prob_n * self.phi / gamma_z + exp_over_flow) * weights, dim=1)  # ELBO
